...
const QuickDetail = (props) => {
  ...
  const uploadButton = (
    <div>
      {uploadLoading ? <LoadingOutlined /> : <PictureOutlined style={{ fontSize: 48, color: '#fff' }} />}
    </div>
  );
  ...
  return (
    <div>
      ...
      <div className="f mdl f-btw" style={{ margin: '40px 0 50px' }}>
        <div style={{ width: '65%' }}>
          <Typography.Text strong style={{ color: '#000' }}>What's your circle name</Typography.Text>
          <Controller
            defaultValue=""
            name="name"
            control={props.form.control}
            render={({ onChange, value }) =>
              <Input placeholder="Circle Name" value={value} onChange={onChange} />
            }
          />
      ...
      <div className="f mdl f-rht">
        ...
        <Button disabled={watchName.length === 0} type="primary" onClick={() => props.onClickNext()} style={{ marginLeft: 30 }}>
          Next Step
        </Button>
      </div>
    </div>
