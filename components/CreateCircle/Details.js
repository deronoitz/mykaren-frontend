  ...
  return (
    <div>
      ...
      <div style={{ margin: '40px 0 50px' }}>
        <Typography.Text strong style={{ color: '#000', marginBottom: 5, display: 'block' }}>About it</Typography.Text>
        <Controller
          defaultValue=""
          name="description"
        ...
        />
        <Typography.Text strong style={{ color: '#000', marginBottom: 5, display: 'block' }}>What we do</Typography.Text>
        <Controller
          defaultValue={[]}
          name="category"
          control={props.form.control}
        ...
        />
        <Typography.Text strong style={{ color: '#000', marginBottom: 5, display: 'block' }}>Circle Profile URL</Typography.Text>
        <Controller
          name="url"
          ...
          render={({ onChange, value }) =>
            <>
              <InputDefault
                ...
                value={value}
                suffix={slugLoading ?
                  <LoadingOutlined style={{ color: '#777' }} /> : !slugError ?
                    <CheckCircleTwoTone twoToneColor="#52c41a" /> :
                    <CloseCircleTwoTone twoToneColor="#eb2f96" />}
              />
              {slugError && <p style={{ marginTop: -20, color: 'var(--primaryColor)' }}>URL already exist. Please change URL value</p>}
            </>
          }
        />

        <Typography.Text strong style={{ color: '#000', display: 'block' }}>Facebook link</Typography.Text>
        <Controller
          name="facebook"
          defaultValue=""
          control={props.form.control}
          render={({ onChange, value }) =>
            <Input style={{ width: '75%', marginBottom: 25 }} onChange={onChange} value={value} placeholder="Your Facebook link" />
          }
        />

        <Typography.Text strong style={{ color: '#000' }}>Instagram account</Typography.Text>
        <Controller
          name="instagram"
          defaultValue=""
          control={props.form.control}
          render={({ onChange, value }) =>
            <Input style={{ width: '75%', marginBottom: 25 }} onChange={onChange} value={value} placeholder="Your Instagram username" />
          }
        />

        <Typography.Text strong style={{ color: '#000' }}>Website</Typography.Text>
        <Controller
          name="website"
          defaultValue=""
          control={props.form.control}
        ...
        <Typography.Text strong style={{ color: '#000', marginBottom: 10, display: 'block' }}>Banner</Typography.Text>
        <style jsx global>
          {`
        ...
        />
        <Typography.Text style={{ fontSize: 12 }}>Karen recommend an image of at least 1920 x 768px for banner.</Typography.Text>
      </div>
