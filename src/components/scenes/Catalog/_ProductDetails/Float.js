  ...
  return (
    <div className="float-wrapper">
      ...
      <div style={{ margin: "8px 0" }}>
        ...
        <Text style={{ marginLeft: 10, fontWeight: 700, color: "var(--titleColor)" }}>
          <span style={{ fontWeight: 400 }}>by</span> {product?.circle.name}
        </Text>
      </div>
      <div className="f mdl" style={{ margin: "27px 0 18px" }}>
        ...
        {product?.stock <= 5 && product?.stock !== 0 && (
          <Text style={{ marginLeft: 20 }}>
            Stock <span style={{ color: "var(--titleColor)", fontWeight: 500 }}>{product?.stock}</span>
          </Text>
        )}
        {product?.stock === 0 && <Text style={{ color: "var(--primaryColor)" }}>Out of stock!</Text>}
      </div>
      <div className="f f-btw mdl" style={{ margin: "13px 0" }}>
        {product?.stock === 0 && <Text>Price</Text>}
        {product?.stock !== 0 && <Text>Sub-total</Text>}
        <Text style={{ fontWeight: 700, fontSize: 18, color: "var(--titleColor)" }}>
          Rp. {new Intl.NumberFormat(["ban", "id"]).format(product?.price)}
        </Text>
      </div>
      {product?.stock !== 0 && (
        <>
