...
const Label = (props) => (
  <Text strong style={{ color: "var(--titleColor)", marginBottom: 2, display: "block" }}>
    {props.children}
  </Text>
);
  ...
  return (
    <div style={style}>
      ...
      <div className="item">
        ...
        <Text>
          {data.rating === 1 ? (
            ...
            <span style={{ color: "#eca80a" }}>PG (Parental Guidence)</span>
          )}
        </Text>
      </div>
      <div className="item">
        <Label>Tags</Label>
        <div>{data.tags?.length > 0 ? data.tags.map((i, index) => <Tag key={index}>{i}</Tag>) : "-"}</div>
      </div>
      <div className="item">
      ...
      <div className="f f-ctr mdl f-c" style={{ marginTop: 30 }}>
        ...
        <br />
        <a className="link lined" onClick={() => setStep(2)}>
          Previous
        </a>
      </div>
    </div>
